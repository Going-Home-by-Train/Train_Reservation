...
# 5. 💣트러블 슈팅
## ERD 구성 과정
### 1. trains 테이블만으로 구성했을 때의 문제점
- trains 테이블 (기차 이름, 출발역, 도착역)
- 출발역과 도착역만을 표시할 수 있었습니다. 이로 인해 기차가 중간역에 정차할 경우, 승객이 중간역에서 내리더라도 그 이후 구간의 빈 좌석을 예약하는 데 어려움이 있었습니다.<br>

### -> 해결 방법
- train_routes 테이블 (경유 순서, 정차 역 이름)을 추가하여 해결
- 기차 정차역을 구분할 수 있었습니다.

### 2. train_routes 테이블 추가 후 (경유 순서, 정차 역 이름)으로 컬럼을 구성했을 때의 문제점
- 각 기차의 중복된 구간을 처리하는 데 어려움이 있었습니다. 예를 들어, 경부선(서울 - 대전 - 부산)과 호남선(서울 - 대전 - 전주) 같은 노선이 있을 경우, 동일한 역을 경유하는 다른 기차들 간의 구간 중복을 관리하는 데 문제가 있었습니다.  

### -> 해결 방법
- train_routes 테이블 (기차 ID, 경유 순서, 정차 역 이름)으로 구성하여 해결
- 기차 호선별로 정차역을 명확하게 구분할 수 있었습니다. 이를 통해 중간역에서 내리더라도, 같은 좌석을 해당 역부터 도착역까지 재예약할 수 있게 되었습니다.
<br>
...
<details>
<summary> 김소연</summary>
 정규표현식을 위한 적절한 select 문제를 만드는게 예상보다 복잡했습니다. 정규표현식의 장점인 복잡한 문자열 패턴을 좀 더 간단하게 처리할 수 있다는 점을 보여주기 위한 문제를 만들기 위해서 정규표현식에 대해 더 공부하고, 연습하게 되었습니다. 그 결과로 정규표현식에 익숙해질 수 있었습니다.<br><br>
...
### 피드백
- 리드미에 권한 부여할 때 *train* -> train.* 으로 바꿔주셔야 할 것 같습니다.
- 정규표현식을 잘 나타낼 수 있는 문제(복잡한 문자열의 패턴 처리 및 추출)가 조금 더 있었으면 좋을 것 같습니다.
- 강사님께 배운 sql문을 바탕으로 문제를 풀 수 있게 한 점이 좋았습니다.
- 정규표현식을 사용하지 않아도 풀 수 있다는 점이 아쉬웠습니다.
- 문제 전반적으로 푸는 재미가 있었습니다😊
- 정규표현식뿐만 아니라 그동안 배운 내용 복습에 최적화된 문제 구성이라고 생각했습니다👍
